import { q, q1, run, tx, isDuplicateObjectError } from "./db.server";
...
export async function seedIfEmpty(): Promise<void> {
  // Idempotent first-boot seed. No cross-isolate lock (session advisory locks
  // are unreliable behind a transaction-mode pooler). If two concurrent cold
  // starts both observe an empty table, they race to insert the demo accounts;
  // the loser's transaction hits the unique email/username constraint and rolls
  // back cleanly — which we treat as "already seeded" since the winner's rows
  // are now present.
  ...
  try {
    await tx(async () => {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        itemIds[name] = id;
        await run(
          `insert into catalog_items (id, name, slug, sort, created_at) values (?,?,?,?,?)`,
          [id, name, name.toLowerCase().replaceAll(" ", "-"), i, t],
        );
    ...
    // A concurrent isolate seeded first: its rows now satisfy the unique
    // constraints we collided on. Safe to treat as done. Re-raise anything else.
