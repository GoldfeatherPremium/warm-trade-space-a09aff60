...
/**
 * A single KPI tile. Renders as a link when `to` is supplied, otherwise a
 * static card. Keeps every dashboard's headline metrics visually identical.
 */
...
/** A titled content panel with an optional header action. */
...
/** A small "see all / manage" style link used in section headers. */
export function SectionAction({ to, label }: { to: string; label: string }) {
  return (
    <Link to={to} className="text-[10px] font-bold text-primary hover:underline whitespace-nowrap">
      {label} →
    </Link>
  );
}

/** Consistent empty state for lists/sections across dashboards. */
...
/** Skeleton grid shown while a dashboard's KPI data loads. */
