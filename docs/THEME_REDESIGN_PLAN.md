...
1. **Theme engine:** `next-themes` (attribute=`class`, `defaultTheme="dark"`, `enableSystem`).
2. **Persistence:** Across devices — `users.theme_pref` DB column + migration; load on session, save on toggle. localStorage still used for instant first paint.
3. **Color direction:** Violet primary; **emerald reserved only for success / "in stock" / "completed"** states. Amber = pending/warning, red = error/failed.
4. **Default theme:** Dark for first-time visitors; "System" selectable in the switcher.
...
**Helpful but not blocking:** 5. Any brand assets — exact logo files, a preferred violet hex, font licenses if changing from Inter/Archivo/JetBrains Mono. 6. Reference screenshots of specific Linear/Stripe/etc. screens you love (so I match the _exact_ vibe, not just the brand name). 7. Priority order if you want a phase pulled forward (e.g., homepage first for a demo). 8. Confirm the work continues on branch `claude/fervent-pasteur-1a1ltv` and whether you want a PR per phase or one big PR.
